package org.souzatech.tasks.infrastructure.exception.handler;

import jakarta.servlet.http.HttpServletRequest;
import org.souzatech.tasks.dto.error.StandardError;
import org.souzatech.tasks.dto.error.ValidationError;
import org.souzatech.tasks.infrastructure.exception.*;
...
public class ResourceHandler {
    ...
    @ExceptionHandler(StatusTaskException.class)
    public ResponseEntity<StandardError> statusTaskException(RuntimeException s,
                                                                     HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(StandardError.builder()
                .timestamp(Instant.now())
                .httpStatus(HttpStatus.CONFLICT)
    ...
    @ExceptionHandler(TaskDepartmentException.class)
    public ResponseEntity<StandardError> taskDepartmentException(RuntimeException s,
                                                             HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(StandardError.builder()
                .timestamp(Instant.now())
                .httpStatus(HttpStatus.CONFLICT)
